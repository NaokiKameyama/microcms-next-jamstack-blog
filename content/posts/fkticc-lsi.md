...
description: "TypeScriptの関数の型付けを、型なし・最低限・関数型の定義・意味のある引数名・typeでの外出しの5段階に分けて解説します。どこまで型を付けるか迷ったときの目安になります。"
...
imageHeight: 674
pr: false
tags: ["TypeScript", "関数", "型定義", "型注釈", "可読性"]
keyPoints:
  - "関数の型付けは、型なし → 引数と戻り値 → 関数型の定義 → 意味のある引数名 → type で外出し、の 5 段階で考える"
  - "引数名を baseValue / increment のように意味が伝わる名前にすると、型を見るだけで関数の役割が分かり、型が設計書の役割を果たす"
  - "type で外出しすると他の関数に流用でき、コードも読みやすくなる"
draft: false
---

関数の型付けは「どこまでやるか」で迷いがちです。この記事では、型なしの状態から、引数と戻り値の型、関数型の定義、意味の伝わる引数名、type での外出しまでを 5 段階に分けて示します。自分のコードをどの段階まで持っていくかの目安にしてください。
...
(ちなみに以下のコードは型付けをしていないのでバグの温床となるリスクが高いです。その理由は[こちら](/blog/7gocwtp2qu_u)で説明しています。)
...
TypeScript を使う理由そのものは「[なぜTypeScriptが必要なのか？](/blog/r5y-oo3lj2d)」で書いています。
